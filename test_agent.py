...
def test_wikipedia_success_returns_answer(mock_dlq):
    ...
    result = run_agent(
        ...
        mock_wikipedia,
        mock_dlq,
        correlation_id="req-success-001",
    )

    assert result["status"] == "SUCCESS"
    assert "North Africa" in result["answer"]
    assert result["correlation_id"] == "req-success-001"
...
def test_wikipedia_failure_redirects_to_dlq(mock_dlq):
    ...
    result = run_agent(
        ...
        mock_wikipedia,
        mock_dlq,
        correlation_id="req-failure-001",
    )

    assert result["status"] == "FAILED_ROUTED_TO_DLQ"
    mock_dlq.assert_called_once_with("Where is Morocco?", "req-failure-001")
